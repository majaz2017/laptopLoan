    ...
    <div class="row">
        <div class="col-lg-6 pb-1">
            <div class="input-group">
                <input type="text" name="searchText" class="form-control" value="{if $searchText != null}{$searchText}{/if}" id="searchText" placeholder="{t}Title, ISBN, Author, Publisher, Genre{/t}">
            ...
            <th class="d-none header-bulk-actions">
                <div class="btn-group" role="group">
                    <button class="btn btn-danger btn-sm" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        <i class="fa fa-trash-o"></i> {t}Delete Books{/t}
                    </button>
                    ...
                    <button class="btn btn-primary btn-sm" id="gen-barcode"><i class="fa fa-magic"></i> {t}Generate Barcode{/t}</button>
                    <button class="btn btn-success btn-sm" data-toggle="modal" data-target="#bulkEditBooks"><i class="fa fa-pencil"></i> {t}Edit Books{/t}</button>
                </div>
            </th>
        ...
        {if isset($books) and $books != null}
            {foreach from=$books item=book name=book}
                <tr>
                    ...
                    <td class="text-center">{$book->getQuantity()-$book->getActualQuantity()}</td>
                    <td class="text-center">
                        <a href="{$routes->getRouteString("bookEdit",["bookId"=>$book->getId()])}" class="btn btn-outline-info btn-sm {if $activeLanguage->isRTL()}ml-1{else}mr-1{/if}" data-container="body" data-toggle="tooltip" title="{t}Edit{/t}"><i class="fa fa-pencil"></i></a>
                        <a href="{$routes->getRouteString("bookClone",["bookId"=>$book->getId()])}" class="btn btn-outline-info btn-sm {if $activeLanguage->isRTL()}ml-1{else}mr-1{/if}" data-container="body" data-toggle="tooltip" title="{t}Clone{/t}"><i class="fa fa-clone"></i></a>
                        <div class="dropdown d-inline" data-trigger="hover" data-toggle="tooltip" title="{t}Delete{/t}">
                            ...
                            <ul class="dropdown-menu {if $activeLanguage->isRTL()}dropdown-menu-left{else}dropdown-menu-right{/if}">
                                ...
                                <li class="divider"></li>
                                <li class="text-center">
                                    <button class="btn btn-outline-secondary delete-book" data-url="{$routes->getRouteString("bookDelete",["bookId"=>$book->getId()])}">
                                        <i class="fa fa-trash-o"></i> {t}Delete{/t}
                                    </button>
                                </li>
                            </ul>
