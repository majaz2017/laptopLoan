<div class="card-header">
    <div class="heading-elements">
        <select name="sortColumn" id="books-sort" class="select-picker {if $activeLanguage->isRTL()}pl-2{else}pr-2{/if} d-tc" autocomplete="off">
                        ...
                        {if $book->getEBookId() != null}
                            <div class="book-list-info">
                                ...
                                {if $siteViewOptions->getOptionValue("showDownloadLink") or (isset($user) and $user->getRole() != null and $user->getRole()->getPriority() > 100)}
                                    <a href="{$routes->getRouteString("electronicBookGet",["electronicBookId"=>$book->getEBookId()])}" class="{if $activeLanguage->isRTL()}mr-1{else}ml-1{/if}"><i class="fa fa-download" aria-hidden="true"></i> {t}Download{/t}</a>
                                {/if}
                                <a href="{$routes->getRouteString("electronicBookViewAdmin",["bookId"=>$book->getId()])}" class="{if $activeLanguage->isRTL()}mr-1{else}ml-1{/if}"><i class="fa fa-eye" aria-hidden="true"></i> {t}Read{/t}</a>
                            </div>
                        {/if}
                    </td>
