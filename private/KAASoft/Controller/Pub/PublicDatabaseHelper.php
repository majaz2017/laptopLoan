...
class PublicDatabaseHelper extends DatabaseHelper
{
    ...
    public function searchPublishers($searchText, $limit = null)
    {
        ...
        $queryResult = $this->kaaSoftDatabase->select(
            ...
            array_merge(
                Publisher::getDatabaseFieldNames(),
    ...
    public function searchBooks($searchText, $offset = null, $perPage = null, $sortBy = "Books.id", $sortOrder = "DESC")
    {
        $result = [];
        $searchCondition = [];
        $limitCondition = [];
        ...
        foreach ($keywords as $keyword) {
            $searchCondition = array_merge(
                $searchCondition,
                ["OR " . $counter => [
                    KAASoftDatabase::$BOOKS_TABLE_NAME . ".ISBN10[~]"       => $keyword,
        ...
        ControllerBase::getLogger()->info($searchCondition);
        $queryResult = $this->kaaSoftDatabase->distinct()->select(
            ...
            array_merge(["AND" => $searchCondition],  $limitCondition)
        );
        ControllerBase::getLogger()->info($queryResult);
        if ($queryResult !== false) {
    ...
    public function filterBooks($query, $countOnly = false, $offset = null, $perPage = null, $sortColumn = null, $sortOrder = null)
    {
        ...
        if ($query->getPublisherIds() !== null and count($query->getPublisherIds()) > 0) {
            ...
            foreach ($query->getPublisherIds() as $publisherId) {
                $publisherCondition = array_merge(
                    $publisherCondition,
        ...
        $queryResult = $this->kaaSoftDatabase->distinct()->$sqlSelectionType(
            KAASoftDatabase::$BOOKS_TABLE_NAME,
            $joinTables,
            $selectedFields,
            array_merge(
                count($searchCondition) == 0 ? [] : ["AND" => $searchCondition],
                $limitCondition
            )
        );
        if ($countOnly) {
    ...
    public function searchPosts($searchText, $limit = null)
    {
        ...
        foreach ($keywords as $keyword) {
            $searchCondition = array_merge(
                $searchCondition,
                ["OR " . $counter => [
                    KAASoftDatabase::$POSTS_TABLE_NAME . ".title[~]"   => $keyword,
    ...
    public function searchPages($searchText, $limit = null)
    {
        ...
        foreach ($keywords as $keyword) {
            $searchCondition = array_merge(
                $searchCondition,
                ["OR " . $counter => [
                    KAASoftDatabase::$PAGES_TABLE_NAME . ".title[~]"   => $keyword,
