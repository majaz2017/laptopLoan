                        ...
                        if ($this->startDatabaseTransaction()) {
                            ...
                            if ($user !== null) {
                                ...
                                foreach ($bookIds as $bookId) {
                                    ...
                                    $request->setCreationDate(date("Y-m-d H:i:s",time()));
                                    ControllerBase::getLogger()->log(LoggerLevel::toLevel(LoggerLevel::FATAL),  $request->getCreationDate() );
                                ...
                                $this->putArrayToAjaxResponse([ "requestIds" => $requestIds ]);
                                // email setup
                                if (count($requests) > 0) {
                                     ControllerBase::getLogger()->log(LoggerLevel::toLevel(LoggerLevel::FATAL),  "in email setup" );
                                    ...
                                    $this->addShortCode("LAST_NAME",
                                                        $user->getLastName());
 ControllerBase::getLogger()->log(LoggerLevel::toLevel(LoggerLevel::FATAL),  "before Request Date" );
                                    $this->addShortCode("REQUEST_DATE",
                                                        Helper::reformatDateTimeString($requests[0]->getCreationDate(),
                                                                                   $this->siteViewOptions->getOptionValue(SiteViewOptions::DATE_FORMAT)));

 ControllerBase::getLogger()->log(LoggerLevel::toLevel(LoggerLevel::FATAL),  "after Request Date" );
                                    ...
                                    $this->addShortCode("BOOKS",
                                                        $booksTemplate);
                                                         ControllerBase::getLogger()->log(LoggerLevel::toLevel(LoggerLevel::FATAL),  "after save" );
                                }
                                // email setup end
                            }
                            else {
