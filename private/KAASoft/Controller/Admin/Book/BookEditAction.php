...
class BookEditAction extends AdminActionBase
{
    ...
    {
        $bookId = $args["bookId"];
        //  ControllerBase::getLogger()->info($args);
        ...
        if (Helper::isAjaxRequest()) {
                ...
                try {
                    if ($this->startDatabaseTransaction()) {
                        ...
                        if (strcmp(
                            $book->getBookSN(),
                            $oldBook->getBookSN()
                        ) !== 0 and $bookDatabaseHelper->isBookSNExist($book->getBookSN())) {
                            $this->rollbackDatabaseChanges();
                        ...
                        ControllerBase::getLogger()->info($book);
                        $date = Helper::reformatDateString($book->getBookPurchaseDate(), $this->siteViewOptions->getOptionValue(SiteViewOptions::DATE_FORMAT));

                        // $book->setBookPurchaseDate($date);
                        ...
                        if (isset($_POST["authors"]) and is_array($_POST["authors"])) {
                            if ($bookDatabaseHelper->saveBookAuthors(
                                $bookId,
                                $_POST["authors"]
                            ) === false) {
                                $this->rollbackDatabaseChanges();
                        ...
                        if (isset($_POST["genres"]) and is_array($_POST["genres"])) {
                            if ($bookDatabaseHelper->saveBookGenres(
                                $bookId,
                                $_POST["genres"]
                            ) === false) {
                                $this->rollbackDatabaseChanges();
                        ...
                        if (isset($_POST["stores"]) and is_array($_POST["stores"])) {
                            if ($bookDatabaseHelper->saveBookStores(
                                $bookId,
                                $_POST["stores"]
                            ) === false) {
                                $this->rollbackDatabaseChanges();
                        ...
                        if (isset($_POST["locations"]) and is_array($_POST["locations"])) {
                            if ($bookDatabaseHelper->saveBookLocations(
                                $bookId,
                                $_POST["locations"]
                            ) === false) {
                                $this->rollbackDatabaseChanges();
                    ...
                    $this->rollbackDatabaseChanges();
                    ControllerBase::getLogger()->error(
                        $e->getMessage(),
                        $e
                    );
                    $errorMessage = sprintf(
                        _("Couldn't save Book.%s%s"),
            ...
            if ($bookId !== null) {
                ...
                if ($book === null) {
                    $this->session->addSessionMessage(
                        sprintf(
                            _("Book with id = '%d' is not found."),
                            $bookId
                        ),
