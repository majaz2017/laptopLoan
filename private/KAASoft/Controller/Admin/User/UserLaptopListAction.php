 ...
 */
class UserLaptopListAction extends AdminActionBase
{
    /**
     * UserLaptopListAction constructor.
    ...
    protected function action($args)
    {
        ...
        if (Helper::isAjaxRequest()) {
            if (Helper::isPostRequest()) { // POST request
                try {
                    if ($this->startDatabaseTransaction()) {
                        ...
                        $result = $userDatabaseHelper->saveUser(
                            $user,
                            !$isLibrarian
                        );
                        if ($result === false) {
            ...
            $this->smarty->assign(
                "lastRequestedBooks",
                $bookHelper->getUserRequestedBooks(
                    $userId,
                    0,
                    10
                )
